...
client.Send(data, data.Length);
    ...
    void ReceiveData() {
        ...
        while (true) {
            ...
            if (parts.Length == 2) {    float x = float.Parse(parts[0], CultureInfo.InvariantCulture);    float y = float.Parse(parts[1], CultureInfo.InvariantCulture);    echoPos = new Vector3(x, y, 0);
            }
        }
    }
